...
instance Monad f => Monad (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f b
  f =<< StateT g =
    StateT $ \s -> (\(a, s') -> (runStateT (f a)) s') =<< g s
...
instance Monad f => Monad (OptionalT f) where
  f =<< OptionalT foa =
  -- does not work:
  -- OptionalT $ (\oa -> (\a -> f a =<< oa)) =<< foa
    OptionalT $ (\oa -> case oa of
                        Empty -> pure Empty
...
distinctG xs =
  runOptionalT $ evalT (f `filtering` xs) S.empty
    where

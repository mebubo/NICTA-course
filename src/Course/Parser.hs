...
list1 p =
  bindParser f p
    where f a = bindParser (\as -> valueParser (a :. as)) $ list p
...
satisfy ::
  ...
  -> Parser Char
satisfy p =
  bindParser f character
    where f c = if p c then valueParser c else unexpectedCharParser c
...
natural =
  bindParser (\ds -> case read ds of
      Empty -> failed
      Full x -> valueParser x) (list1 digit)
...
sequenceParser =
  foldRight f (valueParser Nil)
    where f pa pla = pa `flbindParser` (\a -> pla `flmapParser` (a :.))
...
firstNameParser ::
  Parser Chars
firstNameParser =
  upper `flbindParser` (\a -> ((list lower) `flmapParser` (a :.)))
...
surnameParser =
  ...
  flbindParser (thisMany 5 lower) (\cs1 ->
  flmapParser (list lower) (\cs2 ->
  c :. cs1 ++ cs2)))
...
phoneParser =
  ...
  flbindParser phoneBodyParser (\bs ->
  flmapParser (is '#') (\_ ->
  d :. bs
  )))
...
personParser =
  ...
  spaces1 >>> flbindParser firstNameParser (\fn ->
  spaces1 >>> flbindParser surnameParser (\sn ->
  spaces1 >>> flbindParser smokerParser (\s ->
  spaces1 >>> flbindParser phoneParser (\p ->
